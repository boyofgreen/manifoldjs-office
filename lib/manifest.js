...
//you should have a default version of each of the required ones
  var platformIamges = '128x128, 64x64, 150x150m 50x50';
    ...
    for (var i = 0; i < originalManifest.icons.length; i++) {
      ...
      var patt = new RegExp(icon.sizes);
        if(patt.test(platformIamges)){
            ...
            if(icon.sizes == '150x150') manifest.requireDefaultImg = false;
...
//console.log(data.toString())
    ...
    var totalScope, scopeString;
    if(manifest.mjs_extendedScope) totalScope = [].join(manifest.mjs_extededScope);
    if(manifest.scope)totalScope.push(manifest.scope);
    
    if(manifest.scope || manifest.mjs_extendedScope){
        for(var i=0;totalScope.length > 1;i++){
        tempURL = totalScope[i] 
